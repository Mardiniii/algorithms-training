...
# assume that the speed of the current in the river is negligibly small,
...
# and integer X, returns the earliest time when the frog can jump to the
...
def solution(x, a)
  ...
  a.each_with_index do |element, index|
    next if waypoints[element] == 1

    waypoints[element] ||= 1
    counter += element
    result = index if (route - counter).zero?
  end
