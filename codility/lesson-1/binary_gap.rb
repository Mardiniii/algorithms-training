...
# and contains two binary gaps: one of length 4 and one of length 3. The
# number 20 has binary representation 10100 and contains one binary gap
...
def solution(n)
  binary = n.to_s(2)
  gaps = binary.scan(/1?0+1/)
  return 0 if gaps.empty?

  gaps.map { |gap| gap.gsub('1', '') }.max.length
end
