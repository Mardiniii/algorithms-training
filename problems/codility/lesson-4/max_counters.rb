...
def solution(n, a)
  ...
  a.each do |operation|
    if operation >= 1 && operation <= n
      result[operation - 1] = max_counter if max_counter > result[operation - 1]

      result[operation - 1] += 1

      current_max = result[operation - 1] if current_max < result[operation - 1]
    else
      max_counter = current_max
    end
  end

  result.map { |counter| counter < max_counter ? max_counter : counter}
end
